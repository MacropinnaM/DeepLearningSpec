...
def random_mini_batches(X, Y, mb_size=64, seed=0):
    ...
    num_complete_minibatches = math.floor(m / mb_size)  # number of mini batches of size mb_size in the partitionning
    ...
    if m % mb_size != 0:
        mini_batch_X = shuffled_X[:, num_complete_minibatches * mb_size:]
        mini_batch_Y = shuffled_Y[:, num_complete_minibatches * mb_size:]
        mini_batch = (mini_batch_X, mini_batch_Y)
        mini_batches.append(mini_batch)
